...
public class FragmentAudioRecorder extends Fragment {
    ...
    private void startRecording() {
        ...
        Date date = new Date();
        String nameOfRecordedFile = simpleDateFormat.format(date) + getResources().getString(R.string._3gp);

        mediaRecorder = new MediaRecorder();
        mediaRecorder.setAudioSource(MediaRecorder.AudioSource.MIC);
        mediaRecorder.setOutputFormat(MediaRecorder.OutputFormat.THREE_GPP);
        mediaRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AMR_NB);
        mediaRecorder.setOutputFile(recordedFilePath + "/" + nameOfRecordedFile);
        try {
    ...
    private boolean checkRecordAudioPermission() {
        if (ActivityCompat.checkSelfPermission(requireActivity(), Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) {
            ...
            ActivityCompat.requestPermissions(requireActivity(), new String[]{Manifest.permission.RECORD_AUDIO}, 111);
            return false;
        }
    }
